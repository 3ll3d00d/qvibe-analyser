...
RELEASE_API = 'https://api.github.com/repos/3ll3d00d/qvibe-analyser/releases'
ISSUES_API = 'https://api.github.com/repos/3ll3d00d/qvibe-analyser/issues'
...
class VersionChecker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            if versions is not None and versions[0]['current'] is False or self.__signal_anyway is True:
    ...
    def __convert_to_html(text, issues):
        ...
        def inject_description(m):
            ...
            if int(id)in issues:
                ...
                return f"[{id}](https://github.com/3ll3d00d/qvibe-analyser/issues/{id})"
...
def _get_download_link(assets, os):
    for a in assets:
        if os == 'Windows':
            if a['name'] == 'qvibe-analyser_small.exe':
                return a['url']
        elif os == 'Darwn':
            if a['name'] == 'qvibe-analyser.app.zip':
                return a['url']
