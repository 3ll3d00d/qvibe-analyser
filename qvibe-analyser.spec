...
def get_exe_name():
    '''
    Gets the executable name which is qvibe-analyser for osx & windows and has some dist specific suffix for linux.
    '''
    if platform.system().lower().startswith('linux'):
        linux_dist = distro.linux_distribution(full_distribution_name=False)
        return f"qvibe-analyser_{'_'.join((x for x in linux_dist if x is not None and len(x) > 0))}"
...
if platform.system() == 'Darwin':
    app = BUNDLE(exe,
                 name='qvibe-analyser.app',
                 bundle_identifier='com.3ll3d00d.qvibe-analyser',
